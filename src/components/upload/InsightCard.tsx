...
export function ArchetypeCard({ archetype, confidence, state, index = 0 }: ArchetypeCardProps) {
  return (
    <InsightCard title="Emerging Archetype" state={state} index={index}>
      {archetype ? (
        <div>
          <p className="text-sm text-slate-700">{archetype}</p>
        ...
        <p className="text-sm text-slate-500 italic">Building profile...</p>
      )}
    </InsightCard>
  );
}
...
export function DeepAnalysisPendingCard({ isVisible }: DeepAnalysisPendingCardProps) {
  ...
  return (
    <motion.div
      ...
      className="rounded-xl border-2 border-dashed border-slate-300 p-4 bg-slate-50"
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="font-medium text-slate-700 text-sm">Deep Analysis</h3>
