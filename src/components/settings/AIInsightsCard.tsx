    ...
    <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
      ...
      {loading ? (
        ...
        <div className="text-center py-6">
          <div className="w-12 h-12 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-full flex items-center justify-center mx-auto mb-3">
            <Sparkles className="text-indigo-500" size={24} />
          </div>
          <h4 className="font-medium text-slate-900 mb-1">No insights generated yet</h4>
        ...
        <>
          ...
          <div className="space-y-3">
            {/* Main Stats */}
            <div className="p-4 bg-gradient-to-r from-slate-50 to-blue-50 rounded-lg border border-slate-100">
              <div className="flex items-baseline gap-2 mb-1">
                <span className="text-2xl font-bold text-slate-900">{profilesAnalyzed}</span>
                <span className="text-sm text-slate-600">profiles analyzed</span>
              </div>
            ...
            {(compatibilityScores > 0 || openerSuggestions > 0 || dateIdeas > 0) && (
              <div className="p-3 bg-emerald-50 rounded-lg border border-emerald-100">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-medium text-emerald-700">Value Delivered</span>
                </div>
                <div className="grid grid-cols-3 gap-2 text-center">
                  {compatibilityScores > 0 && (
                    <div>
                      <p className="text-lg font-semibold text-emerald-900">{compatibilityScores}</p>
                      <p className="text-[10px] text-emerald-600">Compatibility scores</p>
                    </div>
                  )}
                  {openerSuggestions > 0 && (
                    <div>
                      <p className="text-lg font-semibold text-emerald-900">{openerSuggestions}</p>
                      <p className="text-[10px] text-emerald-600">Opener ideas</p>
                    </div>
                  )}
                  {dateIdeas > 0 && (
                    <div>
                      <p className="text-lg font-semibold text-emerald-900">{dateIdeas}</p>
                      <p className="text-[10px] text-emerald-600">Date suggestions</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          <button
            ...
            className="w-full flex items-center justify-between px-4 py-2.5 mt-4 text-sm text-slate-700 hover:bg-slate-50 rounded-lg transition-colors min-h-[44px]"
          >
          ...
          {expanded && (
            <div className="mt-4 pt-4 border-t border-slate-100 space-y-4">
              <h4 className="text-xs font-semibold text-slate-700">Insights by Feature</h4>
              ...
              <button
                ...
                className="w-full flex items-center justify-between px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 rounded-lg transition-colors min-h-[44px] border border-slate-100"
              >
