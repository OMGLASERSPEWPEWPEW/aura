...
export function VirtueCompatibilityCard({
  ...
  const getOverallColor = (): string => {
    if (overall_score >= 75) return 'text-emerald-600 dark:text-emerald-400';
    if (overall_score >= 50) return 'text-amber-600 dark:text-amber-400';
    return 'text-red-600 dark:text-red-400';
  };

  // Get overall score background
  const getOverallBg = (): string => {
    if (overall_score >= 75) return 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800';
    if (overall_score >= 50) return 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800';
    ...
    <section className="bg-white dark:bg-slate-800 rounded-xl border border-indigo-200 dark:border-indigo-800 overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/30 dark:to-purple-900/30 p-4 border-b border-indigo-100 dark:border-indigo-800">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-slate-900 dark:text-slate-50">11 Virtues Compatibility</h3>
          </div>
          {onRerun && (
            <button
              onClick={handleRerun}
      ...
      <div className="p-4 space-y-4">
        ...
        <div className={`p-4 rounded-lg border ${getOverallBg()}`}>
          ...
          <p className="text-sm text-slate-600 dark:text-slate-300">{verdictSummary}</p>
        </div>
        ...
        <div className="flex items-center justify-center gap-4 py-2">
          <div className="flex items-center gap-1.5 text-sm">
            ...
            <span className="font-medium text-emerald-700 dark:text-emerald-400">{sympatico_count}</span>
            <span className="text-slate-500 dark:text-slate-400">Sympatico</span>
          </div>
          <div className="flex items-center gap-1.5 text-sm">
            <AlertCircle size={16} className="text-amber-500" />
            <span className="font-medium text-amber-700 dark:text-amber-400">{friction_count}</span>
            <span className="text-slate-500 dark:text-slate-400">Friction</span>
          </div>
          <div className="flex items-center gap-1.5 text-sm">
            <AlertTriangle size={16} className="text-red-500" />
            <span className="font-medium text-red-700 dark:text-red-400">{danger_count}</span>
            <span className="text-slate-500 dark:text-slate-400">Danger</span>
          </div>
        </div>
        ...
        {critical_issues.length > 0 && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3">
            <h4 className="font-bold text-red-800 dark:text-red-300 text-sm mb-2 flex items-center gap-1.5">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 py-2 border-t border-slate-100 dark:border-slate-700"
        >
          {showFullBoard ? (
            <>
