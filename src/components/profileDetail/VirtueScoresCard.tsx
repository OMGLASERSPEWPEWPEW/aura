...
export function VirtueScoresCard({ virtueScores }: VirtueScoresCardProps) {
  ...
  const getScoreColor = (score: number) => {
    ...
    if (score >= 6) return 'bg-teal-400 dark:bg-teal-500';
  ...
  return (
    <section className="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/30 dark:to-orange-900/30 p-5 rounded-xl border border-amber-200 dark:border-amber-700">
      <div className="flex items-start justify-between mb-4">
        <h2 className="text-lg font-bold text-slate-900 dark:text-slate-50 flex items-center gap-2">
          <Landmark size={18} className="text-amber-600 dark:text-amber-400" /> Virtue Match
        </h2>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setShowHelp(!showHelp)}
            className="p-1.5 rounded-full hover:bg-amber-100 dark:hover:bg-amber-800/50 transition-colors"
      ...
      {/* Help Modal */}
      {showHelp && (
        <div className="mb-4 bg-white dark:bg-slate-800 p-4 rounded-lg border border-amber-300 dark:border-amber-600 relative">
          <button
            onClick={() => setShowHelp(false)}
            className="absolute top-2 right-2 p-1 hover:bg-amber-100 dark:hover:bg-amber-800/50 rounded-full"
          >
            <X size={14} className="text-amber-600 dark:text-amber-400" />
          </button>
          <h4 className="font-bold text-amber-900 dark:text-amber-100 text-sm mb-2">What are Partner Virtues?</h4>
        ...
        {virtueScores.map((vs, i) => (
          <div key={i}>
            <div className="flex items-center justify-between mb-1">
              <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{vs.virtue}</span>
              <span className="text-sm font-bold text-slate-800 dark:text-slate-100">{vs.score}/10</span>
            </div>
            <div className="h-2.5 bg-slate-200 dark:bg-slate-600 rounded-full overflow-hidden mb-1">
              <div
                className={`h-full ${getScoreColor(vs.score)} transition-all duration-500`}
                style={{ width: `${vs.score * 10}%` }}
              />
            </div>
            <p className="text-xs text-slate-500 dark:text-slate-400 italic">{vs.evidence}</p>
          </div>
        ))}
      </div>
    </section>
