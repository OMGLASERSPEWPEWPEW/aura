    ...
    <div className="space-y-3">
      ...
      {showStreaming && phase !== 'error' && (
        <>
          ...
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-sm border border-slate-100 dark:border-slate-700 shadow-sm rounded-xl overflow-hidden"
          >
                    ...
                    {profile.identity.name ? (
                      ...
                      >
                        ...
                        {profile.identity.age && (
                          <span className="font-normal text-slate-600 dark:text-slate-400">, {profile.identity.age}</span>
                        )}
                      </motion.h2>
                    ) : (
                      <motion.div
          ...
          {canAbort && (
            <div className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-sm border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-2">
      ...
      {showLegacyLoading && (
        <div className="bg-white dark:bg-slate-800 p-8 rounded-xl shadow-sm text-center">
          <Loader2 className="animate-spin mx-auto mb-4 text-indigo-600 dark:text-indigo-400" size={40} />
          <h3 className="font-semibold text-slate-900 dark:text-slate-50 mb-2">Analyzing Your Profile...</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400">This typically takes 30-60 seconds.</p>
        </div>
      )}
      ...
      {showEmptyState && (
        <div className="bg-white dark:bg-slate-800 p-8 rounded-xl shadow-sm text-center">
          <div className="w-16 h-16 bg-indigo-100 dark:bg-indigo-900/50 rounded-full mx-auto mb-4 flex items-center justify-center">
            <Sparkles className="text-indigo-600 dark:text-indigo-400" size={28} />
          </div>
          <h3 className="font-semibold text-slate-900 dark:text-slate-50 mb-2">No Analysis Yet</h3>
      ...
      {showResults && (
        ...
        >
          {/* Success header */}
          <div className="bg-green-50 dark:bg-green-900/30 p-4 rounded-xl flex items-center gap-3">
            ...
            <button
              onClick={onRunAnalysisLegacy}
              className="ml-auto p-2 text-green-600 dark:text-green-400 hover:bg-green-100 dark:hover:bg-green-900/50 rounded-lg transition-colors"
