...
export function useStreamingAnalysis(): UseStreamingAnalysisReturn {
  ...
  const startAnalysis = useCallback(async (file: File) => {
    ...
    try {
      ...
      // NOTE: Essence generation is now user-triggered for cost control (~$0.04 per image)
      // The virtue sentence (free) is generated when user views profile and clicks "Generate Essence"
      // See ProfileDetail.tsx and ProfileHeader.tsx for manual trigger implementation
